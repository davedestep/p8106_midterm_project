...
Removing the CCS and APR variables provides a lower RMSE and MAE. I think computationally this model is a better choice, and we have evidence that it actually performs better. We only lose 0.2% from our Rsquared value. 
...
```{r}
resamp <-resamples(list(lm = lm.fit2, PCR = pcr.fit2, enet=enet.fit))
summary(resamp)

bwplot(resamples(list(lm = lm.fit2, PCR = pcr.fit2, enet=enet.fit)), metric = "RMSE")

bwplot(resamples(list(lm = lm.fit2, PCR = pcr.fit2, enet=enet.fit, enet2=enet.fit2)), metric = "MAE")
